...
type GenRowFn func(row int) []pgtype.Value
...
type GenValueFn func(row int) pgtype.Value

// RowIdxFn is a GenValueFn that returns the row number as a DInt
func RowIdxFn(row int) pgtype.Value {
	i := pgtype.Int4{}
	i.Set(row)
	return &i
...
func RowModuloFn(modulo int) GenValueFn {
	return func(row int) pgtype.Value {
		i := pgtype.Int4{}
		i.Set(row % modulo)
		return &i
	}
...
func RowModuloShiftedFn(modulo ...int) GenValueFn {
	return func(row int) pgtype.Value {
		i := pgtype.Int4{}
		i.Set(moduloShiftedRecursive(row, modulo))
		return &i
	}
}
...
func RowEnglishFn(row int) pgtype.Value {
	s := pgtype.Text{}
	s.Set(IntToEnglish(row))
	return &s
}
...
func ToRowFn(fn ...GenValueFn) GenRowFn {
	return func(row int) []pgtype.Value {
		res := make([]pgtype.Value, 0, len(fn))
		for _, f := range fn {
