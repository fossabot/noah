...
import (
	"context"
	"fmt"
	"github.com/readystock/noah/db/build"
	"github.com/readystock/noah/db/settings"
	"github.com/readystock/noah/db/util"
	"github.com/readystock/noah/db/util/caller"
	"github.com/readystock/noah/db/util/envutil"
	"github.com/readystock/noah/db/util/sysutil"
...
var (
	...
	ReportSensitiveDetails = envutil.EnvOrDefaultBool("NOAH_REPORT_SENSITIVE_DETAILS", false)
)
...
var crashReportURL = func() string {
	...
	if build.SeemsOfficial() {
		defaultURL = "https://ignored:ignored@errors.readystock.io/sentry"
	}
	return envutil.EnvOrDefaultString("NOAH_CRASH_REPORTS", defaultURL)
}()
