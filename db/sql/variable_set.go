...
func (stmt *VariableSetStatement) Execute(ctx context.Context, ex *connExecutor, res RestrictedCommandResult, pinfo *plan.PlaceholderInfo) error {
	...
	return nil
	target_nodes, err := stmt.getTargetNodes(ctx, ex)
	if err != nil {
		return err
	}

	plans, err := stmt.compilePlan(ctx, ex, target_nodes)
	if err != nil {
		return err
	}
...
func (stmt *VariableSetStatement) getTargetNodes(ctx context.Context, ex *connExecutor) ([]system.NNode, error) {
	return nil, nil
	// return ex.GetNodesForAccountID(nil)
}

func (stmt *VariableSetStatement) compilePlan(ctx context.Context, ex *connExecutor, nodes []system.NNode) ([]plan.NodeExecutionPlan, error) {
