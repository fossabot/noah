...
import (
    ...
    "github.com/readystock/noah/testutils/serverutils"
...
func TestConn(t *testing.T) {
    ...
    // buffer.
    g.Go(func() error {
        return conn.serveImpl(nil, SystemCtx)
    })
...
func processPgxStartup(ctx context.Context, s serverutils.TestServerInterface, c *conn) error {
    ...
    for {
        ...
        query, err := exec.Stmt.Deparse(pg_query.Context_None)
...
func expectExecStmt(
    ...
    if sql, err := es.Stmt.Deparse(pg_query.Context_None); err == nil && *sql != expSQL {
        t.Fatalf("%s: expected %s, got %s", testutils.Caller(1), expSQL, sql)
    }
