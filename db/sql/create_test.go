...
func Test_Create_CompilePlan_Default(t *testing.T) {
	sql := `CREATE TABLE users (id bigserial, email text);`
	...
	assert.Equal(t, plans[0].CompiledQuery, `CREATE TABLE "users" (id bigint, email text)`,
...
func Test_Create_CompilePlan_AccountNoPrimaryKey(t *testing.T) {
	sql := `CREATE TABLE users (id bigserial, email text) TABLESPACE "noah.account";`
	parsed, err := pg_query.Parse(sql)
	if err != nil {
		panic(err)
	}
